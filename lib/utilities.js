...
// 	return this.replace('\b ' + escapeRegExp(search)  + '\b', replacement, 'g')
// }


// String.prototype.replaceAll = function(searchStr, replaceStr) {
// 	return this.replace(searchStr, replaceStr)
// }
...
const replaceSpecialTokens = function(topic, expression, context) {
	...
	if ( !_.isNil(context) && !_.isNil(context['TRIGGER_TOPIC']) ) {
		newExpression = newExpression.replaceAll('$TRIGGER_TOPIC', exports.update_topic_for_expression(context['TRIGGER_TOPIC']))
	}

	if ( !_.isNil(context) && !_.isNil(context['TRIGGER_TIME_UNIX']) ) {
		newExpression = newExpression.replaceAll('$TRIGGER_TIME_UNIX', exports.update_topic_for_expression(context['TRIGGER_TIME_UNIX']))
	}

	if ( !_.isNil(context) && !_.isNil(context['TRIGGER_TIME_STRING']) ) {
		newExpression = newExpression.replaceAll('$TRIGGER_TIME_STRING', exports.update_topic_for_expression(context['TRIGGER_TIME_STRING']))
	}

	if ( !_.isNil(context) && !_.isNil(context['TRIGGER_STRING']) ) {
		newExpression = newExpression.replaceAll('$TRIGGER_STRING', context['TRIGGER_STRING'])
	}
...
exports.prepareExpression = function(topic, expression, context) {
	...
	// var log = false
	// if ( expression == '_deconz_sensors_outdoor_climate_temperature != _outdoors_climate_temperature' ) {
	// 	log = true
	// }
	// if ( log ) { 
