...
const actionProcessor = function(options, context, rule_name, valueOrExpression, topic, callback) { // ISOLATED
	logging.debug('evaluating for publish: ' + topic + '  value: ' + valueOrExpression + '  context: ' + JSON.stringify(context)+ '  options: ' + options)

	utilities.resolveValueOrExpression(rule_name, topic, context, valueOrExpression).then(resultString => {
