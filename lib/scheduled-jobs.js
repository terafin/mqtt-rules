...
const doSchedule = function(rule_name, jobName, cronSchedule, rule, otherwise) {
	...
	if ( !quiet && !global.isTestMode() ) { 
		logging.info('scheduling rule: ' + rule_name + '  job: ' + jobName + '  schedule: ' + cronSchedule + ' timezone: ' + TIMEZONE + ' rule: ' + JSON.stringify(rule)) 
	}
		...
		if (!_.isNil(daily)) {
			...
			Object.keys(daily).forEach(function(daily_key) {
				...
				if (!_.isNil(date)) {
					...
					doSchedule(rule_name, jobKey, newDate.tz(TIMEZONE).toDate(), rule, false)
				}
