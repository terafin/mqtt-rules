...
function evaluateProcessor(job, doneEvaluate) {
    ...
    console.log(' evaluation queue: ' + name, {
    ...
    if (!isOKTime) {
        console.log('not evaluating, bad time =>(' + name + ')', logResult)
    ...
    if (!_.isNil(rule.rules) && !_.isNil(rule.rules.expression)) {
        ...
        jexl.eval(expression, context, function(error, result) {
            console.log('  =>(' + name + ') evaluated expression', Object.assign(logResult, {
        ...
        console.log('  =>(' + name + ') skipped evaluated expression', Object.assign(logResult, {
...
exports.evalulateValue = function(topic, in_context, name, rule) {
    ...
    console.log('enqueued expression evaluation =>(' + queueName + ')', {
