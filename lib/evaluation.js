...
const evaluateProcessor = function(job, doneEvaluate) {
	...
	var shoudLogRule = false

	if ( name.includes('controller_drawer_open')) {
		logging.info(' rule to debug found: ' + name)
		shoudLogRule = true
	}
	...
	if ( context['SCHEDULED_EVENT'] == true ) { 
		logging.info('good time, given it is a scheduled event')
		isOKTime = true 
	}
