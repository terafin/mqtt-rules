...
function evaluateProcessor(job, doneEvaluate) {
    ...
    if (!_.isNil(rule.rules) && !_.isNil(rule.rules.expression)) {
        const expressionHandler = function(expressionString, callback) {
            const expression = utilities.prepareExpression(topic, utilities.update_topic_for_expression(expressionString), context)
...
exports.evalulateValue = function(topic, in_context, name, rule, isScheduledJob) {
    ...
    job.context_value['TRIGGER_TOPIC'] = topic
        ...
        job.context_value['TRIGGER_TOPIC_COMPONENTS'] = topicComponents
    }
    if (evaluateAfter > 0)
