...
function evaluateProcessor(job, doneEvaluate) {
    ...
    console.log('eval queue: ' + name + '    begin')
    ...
    if (!_.isNil(allowed_times)) {
        ...
        allowed_times.forEach(function(timeRange) {
            ...
            const result = moment(new Date()).tz('America/Los_Angeles').isBetween(startDate.tz('America/Los_Angeles'), endDate.tz('America/Los_Angeles'))
            if (result == true) {
                isOKTime = true
            }
        }, this)
