...
function evaluateProcessor(job, doneEvaluate) {
    ...
    const currentDate = new Date().tz(TIMEZONE)
    ...
    if (!_.isNil(allowed_times)) {
        ...
        allowed_times.forEach(function(timeRange) {
            ...
            const result = moment(currentDate).isBetween(startDate.tz(TIMEZONE), endDate.tz(TIMEZONE))
            if (result == true) {
                isOKTime = true
            }
        }, this)
    ...
    if (!isOKTime) {
        logging.info('not evaluating, bad time (' + currentDate + ')  =>(' + name + ')', logResult)
    ...
    logging.info('evaluating, good time  (' + currentDate + ') =>(' + name + ')', logResult)
...
exports.evalulateValue = function(topic, in_context, name, rule) {
    ...
    if (evaluateAfter > 0)
        logging.info('enqueued expression evaluation =>(' + rule + ')    for: ' + evaluateAfter)
