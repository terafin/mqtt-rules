...
exports.handleNotification = function(name, topic, context, notify_config) {
    ...
    utilities.resolveValueOrExpression(name, topic, context, notify_config.message).then(result => {
        notify_message = result
        logging.info(' resolved message: ' + result)

        utilities.resolveValueOrExpression(name, topic, context, notify_config.title).then(result => {
            logging.info(' resolved title: ' + result)
            notify_title = result
...
exports.notificationProcessor = function(options, context, rule_name, notify_config, notify_name, callback) { // ISOLATED
    ...
    utilities.resolveValueOrExpression(rule_name, '', context, publishExpression).then(publishResult => {
        logging.info(' shoudl notify: ' + publishExpression)
