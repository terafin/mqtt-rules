...
exports.handleNotification = function(name, notify_config) {
    ...
    if (_.isNil(notify_config.message)) {
        console.log(' Empty notification message', notify_config)
        return
    }
    ...
    p.send(msg, function(err, result) {
        ...
        if (!_.isNil(err)) {
            json.error = err
            console.log(' failed notification', json)
        } else {
            console.log(' successfully notified', json)
        }
    })
}

exports.notificationProcessor = function(context, rule_name, notify_config, notify_name, callback) { // ISOLATED
    ...
    console.log('  start evaluating publish expression for =>(' + rule_name + ')', {
    ...
    jexl.eval(publishExpression, context,
        function(publishError, publishResult) {
            console.log('  done evaluating publish expression for =>(' + rule_name + ')', {
